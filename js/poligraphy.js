...
function poligraphy(){
	...
	setTimeout(function() {
		...
		.animate({top:'15%'},500, correctSliders())
		.append('<div class="car"></div>');
		//Выхлоп машинки
		for (var i = 1; i < 6; i++) {
			$(".car").append('<div class="dim'+i+'"></div>');
		}	
		//Шарик, стрелки и мячик
		$(".bgslider")
		.append('<div class="sharik"></div>')
		.append('<div class="mjach"></div>')
		.append('<div class="back_poli" title="Back to Other"></div>')
		.append('<div class="next_poli" title="Go to Websites"></div>');
		//Нитка от шарика
		$(".sharik")
	...
	setTimeout(function() {
		...
		setTimeout(function() {
			for (var i = 1; i < 5; i++) {
				...
				$(".next_poli, .back_poli").delay(1000).fadeIn();
			}
		}, 2000);	
	...
	$(".bgslider").on("mouseover", ".back_poli", function(){
		$(this).effect( "shake", { direction: "left", times: 10, distance: 1}, 500 );
	});
	$(".bgslider").on("mouseover", ".next_poli", function(){
		$(this).effect( "shake", { direction: "left", times: 10, distance: 1}, 500 );
	});
	$(".bgslider").on("click", ".back_poli", function(){
		location.hash = "#other";
	});
	$(".bgslider").on("click", ".next_poli", function(){
